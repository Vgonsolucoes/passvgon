...
export default function LoginForm({
  ...
  useEffect(() => {
    let alive = true;
    fetch("/api/auth/csrf", { cache: "no-store", credentials: "include" })
      .then((r) => r.json().catch(() => ({})))
      .then((data) => {
        if (alive) {
  ...
  const errorMessage = useMemo(() => {
    ...
    switch (initialError) {
      ...
      case "MissingCSRF":
        return "Sessão de login expirada. Atualize a página e tente novamente.";
      default:
        return `Erro ao entrar: ${initialError}`;
    }
  }, [initialError]);
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 overflow-hidden">
          <div className="p-8">
            ...
            <form
              className="space-y-4" action="/api/auth/callback/credentials" method="POST">
              <input type="hidden" name="csrfToken" value={csrfToken} />
              <input type="hidden" name="callbackUrl" value={initialCallbackUrl} />
              <input type="hidden" name="json" value="true" />
              <div>
                <label
                  htmlFor="email" className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">
                  E-mail
                </label>
                <input
                  id="email"
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  <label
                    htmlFor="password" className="block text-sm font-medium text-slate-700 dark:text-slate-300">
                    Senha
                  </label>
                  <a
                    href="#"
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full px-4 py-2.5 rounded-lg bg-blue-700 hover:bg-blue-800 text-white font-medium transition-colors shadow-lg shadow-blue-700/20 disabled:opacity-50 disabled:cursor-not-allowed">
